...
const InfoSection = ({ title, description, end, match, rewardKudosAll }: Prediction) => {
  return (
    <Fragment>
      <div className="flex justify-between items-center">
        <div>
          <p className="text-24 text-sf-gray-300 font-bold leading-16">{title}</p>
          <p className="text-14 text-sf-neutral-300 font-poppins leading-20 mt-8">{match?.title}</p>
        </div>
        <div className="flex">
          <div className="flex items-center drop-shadow-[0_0_12px_rgba(204,143,0,1)]">
            ...
            <div className="flex flex-col justify-between ml-8 md:ml-12">
              ...
              <span className="point-value font-bold md:text-24 xl:text-28 leading-30">{rewardKudosAll}</span>
            </div>
          </div>
        </div>
      </div>
