...
interface GameAliveModalProps extends ModalProps {
  detail: any
}

const GameAliveModal = ({ show, onClose, detail }: GameAliveModalProps) => {
  return (
    <Modal show={show} onClose={onClose}>
      <div className="w-full max-w-400 bg-sf-zinc-900 rounded-[4px] flex flex-col items-center pt-50 pb-34 px-20 sm:px-50">
        <p className="w-full text-22 font-poppins font-bold text-center text-golden">Game On!</p>
        ...
        <Link
          href={`/play/game/${
            detail?.section === GameTypeEnum.Predict
              ? 'predict'
              : detail?.section === GameTypeEnum.Trivia
              ? 'trivia'
              : 'milestone'
          }/${detail.id}`}
        >
